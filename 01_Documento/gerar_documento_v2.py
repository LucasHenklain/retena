...
Saída: Retena_Fase6_Documento_v2.docx (mesma pasta). Versão 2: acrescenta 3.7, 3.8 e 4.9. A conversão para PDF é feita via Word (COM), fora deste script.
...
OUT_DOCX = os.path.join(AQUI, "Retena_Fase6_Documento_v2.docx")
...
def sec_capa():
    ...
    p("São Paulo, setembro de 2026 · Versão 2 (09/09/2026): benchmark de modelos, portabilidade OULAD e caminho OCI", alinh="c", tamanho=10, cor=GRAFITE, depois=2)
    page_break()
...
def sec_v2_parte3():
    """Seções acrescentadas na versão 2 (incorporação do EduRetain)."""
    h2("3.7 Benchmark de modelos (v2)")
    p("Após a entrega da versão 1, a equipe incorporou o pipeline EduRetain (contribuição de um integrante, construído "
      "sobre o dataset público OULAD) como fonte de configurações alternativas de modelo. O benchmark abaixo aplica, "
      "sobre a mesma base aluno-semana e o mesmo split temporal do MVP (treino < jun/2026 com embargo de 21 dias, teste "
      "jun–ago/2026), as configurações do EduRetain — regressão logística L1 balanceada (liblinear, C=1), XGBoost com "
      "scale_pos_weight calculado e GradientBoosting padrão — e a seleção por F2. O modelo entregue (HGB) não foi "
      "alterado; o script 07_benchmark_modelos.py reproduziu a AUC-ROC de referência (0,882).", alinh="j")
    tabela(
        ["Modelo", "AUC-ROC", "AUC-PR", "Prec. @ top-20%", "Recall @ 0,5", "F2"],
        [
            ["HGB (entregue)", "0,882", "0,839", "0,882", "0,580", "0,626"],
            ["Regressão logística baseline (03)", "**0,893**", "**0,859**", "**0,919**", "0,659", "0,696"],
            ["Regressão logística baseline + balanced", "0,889", "0,855", "0,904", "**0,784**", "**0,766**"],
            ["Regressão logística EduRetain (L1, balanced)", "0,882", "0,850", "0,904", "0,767", "0,759"],
            ["XGBoost (configuração EduRetain)", "0,874", "0,818", "0,866", "0,539", "0,586"],
            ["GradientBoosting (padrão scikit-learn)", "0,811", "0,739", "0,764", "0,498", "0,543"],
        ],
        larguras=[5.2, 1.9, 1.9, 2.4, 2.2, 1.6], legenda="Benchmark no conjunto completo de teste (n = 1.609; positivos 28,8%)", primeira_negrito=True,
    )
    tabela(
        ["Modelo", "AUC-ROC", "AUC-PR", "Prec. @ top-20%", "F2"],
        [
            ["HGB (entregue)", "0,743", "0,460", "0,377", "0,209"],
            ["Regressão logística baseline (03)", "**0,772**", "**0,494**", "**0,396**", "0,232"],
            ["Regressão logística baseline + balanced", "0,765", "0,475", "0,388", "**0,475**"],
            ["Regressão logística EduRetain (L1, balanced)", "0,745", "0,461", "0,373", "0,452"],
            ["XGBoost (configuração EduRetain)", "0,737", "0,442", "0,350", "0,254"],
            ["GradientBoosting (padrão scikit-learn)", "0,681", "0,287", "0,285", "0,169"],
        ],
        larguras=[6.4, 2.2, 2.2, 2.8, 1.6], legenda="Benchmark no subconjunto acionável (alunos com atividade nos 28 dias anteriores; n = 1.298; positivos 14,7%)", primeira_negrito=True,
    )
    figura(os.path.join(FIGS_MVP, "fig_benchmark_modelos.png"), "AUC-PR e F2 por modelo, conjunto completo e subconjunto acionável (07_benchmark_modelos.py)")
    h3("Leitura")
    bullets([
        "**Nenhuma configuração do EduRetain supera o baseline logístico já presente no pipeline.** A regressão L1 balanceada fica em AUC-PR 0,850 (completo) e 0,461 (acionável) contra 0,859 e 0,494 do baseline; o XGBoost com a configuração do EduRetain (0,818 / 0,442) fica abaixo do HGB; o GradientBoosting padrão é o pior do grupo. Essas configurações foram desenhadas para o OULAD (features cadastrais, ~20 mil linhas) e não convergem no mesmo nível com 2.126 linhas de treino.",
        "**A vantagem em F2 é efeito de limiar, não de ordenação.** Modelos com class_weight balanced ou scale_pos_weight inflacionam as probabilidades e cruzam o limiar 0,5 com mais frequência (recall 0,784 na logística balanceada contra 0,580 no HGB), mas suas métricas de ordenação são iguais ou piores. Movendo o limiar do HGB para 0,30 — a fronteira da faixa Médio, fixada a priori — o F2 sobe de 0,626 para 0,643 sem retreinar. É por isso que o Retena opera com faixas (0,30 e 0,60) em vez de um limiar único.",
        "**Se o critério fosse estritamente AUC-PR ou F2, a regressão logística venceria por margem pequena e já conhecida** (0,020 em AUC-PR no completo; 0,034 no acionável), dentro da incerteza amostral de ~200 alunos com linhas correlacionadas. Não há vencedor com folga.",
        "**O HGB permanece o modelo entregue** porque reproduz exatamente o reportado, tem a melhor precisão a 0,5 (0,924 — falso positivo custa tempo de tutor), lida com interações sem reescalonamento e preserva a rastreabilidade entre metadados, métricas, figuras e dashboard. O que o benchmark acrescenta de acionável é a evidência de que o limiar 0,30 recupera o recall dos modelos balanceados, reforçando a política de faixas.",
    ])
    p("Limitações: um único split temporal, sem intervalos de confiança; métricas em limiar fixo dependem da calibração; "
      "nenhum candidato foi tunado. Próximo passo: teste pareado por corte (9 cortes) ou bootstrap por aluno. Artefatos: "
      "05_MVP/outputs/benchmark_modelos.json e .md; 05_MVP/pipeline/07_benchmark_modelos.py.", alinh="j")
    sec_v2_oulad()


def sec_v2_oulad():
    h2("3.8 Portabilidade do método: OULAD (v2)")
    p("Para testar se o método funciona em outra instituição sem mudar arquitetura nem hiperparâmetros, o pipeline foi "
      "reimplementado sobre o OULAD (Open University Learning Analytics Dataset, Reino Unido): 7 módulos, 22 apresentações, "
      "32.593 matrículas e 10,66 milhões de linhas de cliques no ambiente virtual. A base aluno-semana resultante tem "
      "730.673 linhas de 24.392 alunos, com o mesmo rótulo (inativo_21d = zero cliques nos 21 dias seguintes ao corte), "
      "features análogas (cliques e dias ativos em 7/14/28 dias, recência, acumulados, regularidade semanal, entregas) e o "
      "mesmo HistGradientBoosting. Split temporal por apresentação: treino em 2013B e 2013J (11.438 alunos), teste em "
      "2014B e 2014J (14.293 alunos). Scripts e resultados em 05_MVP/portabilidade_oulad.", alinh="j")
    tabela(
        ["Métrica (rótulo inativo_21d, teste 2014)", "IES Demo (Retena)", "OULAD — completo", "OULAD — acionável"],
        [
            ["Alunos no teste / linhas", "183 / 1.609", "14.293 / 413.072", "— / 354.715"],
            ["Taxa de positivos", "28,8%", "20,6%", "10,6%"],
            ["AUC-ROC (HGB)", "0,882", "**0,922**", "0,853"],
            ["AUC-PR (HGB)", "0,839", "0,813", "0,418"],
            ["Precisão @ top-20%", "0,882", "0,724", "0,342"],
            ["Recall capturado no top-20%", "0,612", "0,701", "0,647"],
            ["Lift @ top-20%", "3,06×", "**3,51×**", "3,23×"],
            ["Faixa Alto (≥ 0,60): inatividade observada", "95,2%", "85,0%", "—"],
            ["Faixa Baixo (< 0,30): inatividade observada", "14,3%", "6,4%", "—"],
        ],
        larguras=[5.6, 3.2, 3.6, 3.6], legenda="Método Retena na IES Demo e no OULAD (mesmos hiperparâmetros; split temporal nos dois casos)", primeira_negrito=True,
    )
    figura(os.path.join(ENTREGA, "05_MVP", "portabilidade_oulad", "figs", "fig_oulad_roc_pr.png"), "Curvas ROC e Precisão-Recall no OULAD (teste 2014B + 2014J), conjunto completo e subconjunto acionável")
    h3("Leitura")
    bullets([
        "**O ranqueamento se manteve ou melhorou em outra instituição, outro país e outro LMS:** AUC-ROC 0,922 e lift 3,51× no OULAD contra 0,882 e 3,06× na IES Demo, sem nenhum reajuste. AUC-PR e precisão caem em termos absolutos porque a taxa de positivos é menor (20,6% contra 28,8%); o lift, que corrige pela base, subiu.",
        "**As mesmas features dominam:** dias ativos em 28 dias, recência e regularidade semanal lideram a importância por permutação, com o mesmo sinal dos coeficientes da regressão logística nas duas bases. Volume de cliques vale menos que constância.",
        "**As faixas fixadas a priori continuam separando risco:** 85% de inatividade observada na faixa Alto contra 6% na Baixo; com 400 mil linhas a calibração fica quase diagonal.",
        "**Estabilidade:** AUC-ROC entre 0,858 e 0,954 nos 14 módulos-apresentações de teste; o módulo CCC, que só existe em 2014 e nunca foi visto no treino, obteve 0,931 e 0,906.",
        "**O que continua difícil:** prever quando a desmatrícula administrativa acontece (janela de 28 dias, 2,6% de positivos) dá AUC-ROC 0,73 e precisão de 6% no top-20% — reforça a escolha do rótulo comportamental como gatilho de ação; a desmatrícula é o desfecho a evitar, não o alvo que se prevê com precisão semana a semana.",
    ])
    h3("Comparação com o protocolo do EduRetain no mesmo dataset")
    p("O setup do EduRetain (alvo final_result = Withdrawn, features até o dia 30, regressão logística L1 balanceada) foi "
      "reproduzido no OULAD com split aleatório 80/20, como no original, e com split temporal 2013 → 2014. A inflação do "
      "split aleatório é real, mas modesta (+0,024 em AUC-ROC para a logística). O achado relevante é outro: 15,7% das "
      "matrículas já tinham desmatrícula registrada até o dia 30 e são acertadas trivialmente pelo modelo; retirando-as do "
      "teste, o desempenho cai para AUC-ROC 0,66–0,69 e AUC-PR 0,32–0,34. É o análogo direto do subconjunto acionável do "
      "Retena, onde o método semanal com recência e regularidade obteve 0,853 de AUC-ROC no mesmo dataset e mesmo split. "
      "Os alvos são diferentes por construção, mas o resultado mostra que o grão semanal carrega muito mais sinal acionável "
      "do que uma fotografia única do dia 30. Recomendação incorporada ao roadmap: excluir do treino e da avaliação as "
      "matrículas já encerradas no corte, e reportar sempre o número acionável ao lado do global.", alinh="j")
    tabela(
        ["Cenário (alvo Withdrawn, features até o dia 30)", "Modelo", "AUC-ROC", "AUC-PR", "Precisão @ top-20%"],
        [
            ["(a) Split aleatório 80/20 estratificado", "Logística EduRetain", "0,814", "0,714", "0,780"],
            ["(a) Split aleatório 80/20 estratificado", "HGB Retena", "0,834", "0,748", "0,807"],
            ["(b) Split temporal 2013 → 2014", "Logística EduRetain", "0,790", "0,700", "0,772"],
            ["(b) Split temporal 2013 → 2014", "HGB Retena", "0,821", "0,754", "0,828"],
            ["(c) Temporal, sem quem já se desmatriculou até o dia 30", "Logística EduRetain", "0,659", "0,315", "0,334"],
            ["(c) Temporal, sem quem já se desmatriculou até o dia 30", "HGB Retena", "0,692", "0,336", "0,376"],
        ],
        larguras=[6.2, 3.2, 2.0, 2.0, 2.6], legenda="Protocolo EduRetain reproduzido no OULAD: efeito do split e dos alunos já desmatriculados", primeira_negrito=True,
    )
    p("Limitações do estudo: tempo relativo ao início da apresentação (sem calendário real), linhas correlacionadas (n efetivo "
      "da ordem de 15 mil matrículas de teste), apenas dois anos para o split temporal e cortes finais com inatividade que "
      "não é risco (fim do conteúdo). Tempo total de execução: cerca de 110 segundos em um notebook.", alinh="j")


def sec_v2_oci():
    h2("4.9 Caminho OCI executável (v2)")
    p("A versão 1 provou a arquitetura in-database no Oracle AI Database 26ai Free em Docker e deixou o Autonomous como "
      "roadmap. A versão 2 incorpora, adaptado do pipeline EduRetain (contribuição de um integrante da equipe, construído "
      "sobre Object Storage + Autonomous Database), um caminho operacional completo do ambiente local para a OCI, na pasta "
      "06_Oracle/oci. Por honestidade: esta máquina não tem tenancy nem credenciais OCI, portanto tudo o que depende da "
      "nuvem está documentado como runbook e marcado como não executado; o que pôde ser executado contra o Free local foi "
      "executado e a saída está colada no README da pasta e em evidencia_merge.txt.", alinh="j")
    tabela(
        ["Artefato (06_Oracle/oci)", "O que faz", "Status"],
        [
            ["README.md", "Runbook \"do Oracle Free local ao Autonomous AI Database em 60 minutos\": provisionar o Always Free, wallet mTLS × TLS, usuário RETENA com CREATE MINING MODEL, bucket e upload, DBMS_CLOUD, execução dos scripts 01–07 na ordem, DBMS_SCHEDULER, APEX e Select AI; tabela \"Local (Free) → Autonomous\"", "Runbook (partes OCI não executadas)"],
            ["conexao.py", "Resolve a conexão por variáveis de ambiente: modo local (padrão) ou Autonomous (wallet ou TLS), sem credencial no código; comando --teste imprime banner e SYSTIMESTAMP", "**Executado** no Free local: banner 26ai Free 23.26.3.0.0; 684.723 eventos; 203 alunos no snapshot; 3 modelos OML"],
            ["exportar_para_object_storage.py", "Exporta EVENTOS_LMS (ou o parquet) para CSV gzip e faz upload ao bucket via SDK oci; --dry-run gera só o arquivo", "**Dry-run executado**: 684.723 linhas, 3,54 MiB, mesmo SHA-256 nas duas fontes; upload não executado"],
            ["sql/08_dbms_cloud_ingestao.sql", "DBMS_CLOUD.CREATE_CREDENTIAL + COPY_DATA para EVENTOS_LMS, alternativa com tabela externa e job de ingestão", "Conferido pelo divisor de statements (16 statements); só roda no Autonomous — não executado"],
            ["sql/09_merge_risco_snapshot.sql", "MERGE idempotente de VW_RISCO_ALUNO_ATUAL para RISCO_ALUNO_SNAPSHOT e RISCO_ALUNO_HISTORICO (chave aluno + corte), padrão EduRetain", "**Executado 2× no Free local**: 19/19 statements, 203 → 203 linhas, 0 duplicatas (evidencia_merge.txt)"],
            ["executar_sql_oci.py", "Roda os .sql existentes com o runner original, mas com a conexão do conexao.py (necessário para wallet)", "**Executado** (script 09)"],
        ],
        larguras=[3.6, 7.2, 5.2], legenda="Caminho OCI: artefatos, função e status de execução", primeira_negrito=True,
    )
    h3("O que muda do local para o Autonomous")
    bullets([
        "**Ingestão:** o CSV exportado do LMS vai para o Object Storage e entra no banco por DBMS_CLOUD.COPY_DATA, eliminando o carregador Python; a verificação esperada é a mesma da evidência local (684.723 eventos, 203 alunos, 15/01 a 26/08/2026).",
        "**Conexão:** wallet mTLS por padrão no Always Free (alias _high), ou TLS sem wallet quando habilitado; o conexao.py escolhe pelo .env, e os scripts 01–09 rodam sem alteração.",
        "**Operação:** o MERGE idempotente permite reprocessar o snapshot de risco toda semana sem duplicar linhas e mantém o histórico por corte, base para o painel APEX e para o retreino agendado com DBMS_SCHEDULER.",
        "**Próximo passo antes do NEXT:** executar o runbook com a conta da equipe (estimativa de 60 minutos) e substituir as evidências locais por evidências no Autonomous; os números esperados já estão documentados para conferência.",
    ])
